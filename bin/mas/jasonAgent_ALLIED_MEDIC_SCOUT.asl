...
+starting_position(Sx, Sy, Sz) <-
	.println( "De camino a la posicion de comandancia ", Sx, " ", Sy, " ", Sz );
	...
	.my_team("ALLIED", E);
	.concat( "starting_position(", Sx, ",", 0, ",", Sz, ")", Messg );
 ...
 * 
 */
+!get_agent_to_aim .		// Purely defensive Medics
...
+!perform_look_action : position_bug <-
	...
	-+tasks([]);
	?my_position(X, Y, Z);
	+starting_position(math.round(X)+20, Y, math.round(Z)+20);
	-position_bug
	.
