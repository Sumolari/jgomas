 ...
 */
+!sparse_team( TEAM ) <-
	.println( "Dispersandose de algun miembro del equipo: ", TEAM );
	-+sparsing( TEAM )
	.
...
+!sparse : sparsing( TEAM ) & TEAM > 0 <-
	...
	while ( auxC( C ) & L > C & targetFound( "NO" ) ) {
		...
		if ( Equipo == TEAM ) {
			...
			.println( "Voy hacia: ", pos( X - 50, Y, Z + 50 ) );
			-+targetFound( "SI" );
		}
		-+auxC( C + 1 );
	}
...
+!fw_add_task( Task, Priority ) : currentActionPriority( CurrentPriority ) <-
	!add_task( Task );
